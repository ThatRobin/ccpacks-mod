...
import io.github.connor3001.ccpacks.dataDrivenTypes.DDFallingBlock;
...
public class MMFallingBlock {

    public MMFallingBlock(Identifier identifier, String material, String tool, String sound, Boolean collision, Integer hardness, Integer resistance, Float slipperiness, int luminance, int miningLevel, Consumer<Entity> action, ConditionFactory<LivingEntity>.Instance condition, Identifier loot){
        Material mat = getMat(material);
        BlockSoundGroup sounds = getSound(sound);
        Tag<Item> tools = getTool(tool);
        DDFallingBlock EXAMPLE_BLOCK = new DDFallingBlock(FabricBlockSettings.of(mat).collidable(collision).strength(hardness, resistance).slipperiness(slipperiness).luminance(luminance).sounds(sounds).breakByTool(tools).requiresTool().drops(loot), action, condition);
