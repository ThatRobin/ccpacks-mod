...
import io.github.connor3001.ccpacks.dataDrivenTypes.DDStairBlock;
...
public class MMStairBlock {

    public MMStairBlock(Identifier identifier, String material, String tool, String sound, Boolean collision, Integer hardness, Integer resistance, Float slipperiness, int luminance, int miningLevel, Consumer<Entity> action, ConditionFactory<LivingEntity>.Instance condition, Identifier loot, Block block){
        Material mat = getMat(material);
        BlockSoundGroup sounds = getSound(sound);
        Tag<Item> tools = getTool(tool);
        BlockState state = block.getDefaultState();
        DDStairBlock EXAMPLE_BLOCK = new DDStairBlock(state, FabricBlockSettings.of(mat).collidable(collision).strength(hardness, resistance).slipperiness(slipperiness).luminance(luminance).sounds(sounds).breakByTool(tools).requiresTool().drops(loot), action, condition);
