...
public class CCPackClientRegistry {
    ...
    private void register(List<Pair<SerializableData.Instance, JsonObject>> list){
            ...
            try {
                if (type.equals("ccpacks:item")) {
                    ...
                    if (itemType.equals("generic")) {
                        ...
                        CCPacksMain.LOGGER.info(instance2.getId("identifier"));
                        ...
                        Item CUSTOM_MUSIC_DISC = new DDMusicDiscItem(instance2.getInt("comparator_output"), (SoundEvent) instance2.get("sound"), (new Item.Settings().maxCount(1).group(ItemGroup.MISC)));
                    ...
                    instance2 = SerializableObjects.enchantmentData.read(jsonObject);
                ...
                new FatalErrorScreen(new LiteralText("CCpacks has ran into an error!" + e.getCause()), new LiteralText(e.getMessage()));
            }
        }
        ...
        for(int i = 0; i < list.size(); i++) {
            ...
            if (type.equals("ccpacks:portal")) {
                ...
                } else if(entityType.equals("sheep")) {
                    instance2 = SerializableObjects.genericEntityData.read(jsonObject);

                    EntityType<DDSheepEntity> entity = FabricEntityTypeBuilder.create(SpawnGroup.CREATURE, DDSheepEntity::new).dimensions(EntityDimensions.fixed(0.6F, 1.8F)).build();
                    Registry.register(Registry.ENTITY_TYPE, instance2.getId("identifier"), entity);
                    FabricDefaultAttributeRegistry.register(entity, SheepEntity.createMobAttributes());

                    EntityRendererRegistry.INSTANCE.register((EntityType<DDSheepEntity>)(Registry.ENTITY_TYPE.get(instance2.getId("identifier"))), (context) -> new DDSheepEntityRenderer(context, instance2.getId("texture")));
                }
                ...
                if (itemType.equals("shield")) {
                    instance2 = SerializableObjects.shieldData.read(jsonObject);
            ...
            } else if(type.equals("ccpacks:biome")) {
                instance2 = SerializableObjects.biomeData.read(jsonObject);

                ConfiguredSurfaceBuilder<TernarySurfaceConfig> BIOME_SURFACE_BUILDER = SurfaceBuilder.DEFAULT
                        .withConfig(new TernarySurfaceConfig(
                                ((Block) instance2.get("primary_block")).getDefaultState(),
                                ((Block) instance2.get("secondary_block")).getDefaultState(),
                                ((Block) instance2.get("tertiary_block")).getDefaultState()));
                Biome OBSILAND = createBiome(BIOME_SURFACE_BUILDER);
                RegistryKey<Biome> OBSILAND_KEY = RegistryKey.of(Registry.BIOME_KEY, instance2.getId("identifier"));
                Registry.register(BuiltinRegistries.CONFIGURED_SURFACE_BUILDER, instance2.getId("identifier"), BIOME_SURFACE_BUILDER);
                Registry.register(BuiltinRegistries.BIOME, OBSILAND_KEY.getValue(), OBSILAND);
                for (int e = 0; e < ((List<OverworldClimate>) instance2.get("climates")).size(); e++) {
                    OverworldBiomes.addContinentalBiome(OBSILAND_KEY, ((List<OverworldClimate>) instance2.get("climates")).get(e), ((List<Double>)instance2.get("weight")).get(e).doubleValue());
                }
            }
    ...
    private static Biome createBiome(ConfiguredSurfaceBuilder<TernarySurfaceConfig> OBSIDIAN_SURFACE_BUILDER) {
        ...
        try (Stream<Path> paths = Files.walk(Paths.get(pack.getPath()))) {
            paths.forEach((file) -> {
                ...
                if (string3.endsWith(".json")) {
                    ...
                    instance = SerializableObjects.getType.read(jsonObject);
        ...
        while(enumeration.hasMoreElements()) {
            ...
            if (!zipEntry.isDirectory()) {
                ...
                if (string3.endsWith(".json") && string3.startsWith(string2)) {
                    ...
                    SerializableData.Instance instance = SerializableObjects.getType.read(jsonObject);
                    registerElements(jsonObject, instance);
                }
            }
        }
